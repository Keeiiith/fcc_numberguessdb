...
GAMES_PLAYED=$($PSQL "SELECT COUNT(*) FROM users INNER JOIN games ON users.user_id = games.user_id WHERE username='$USERNAME'")
BEST_GAME=$($PSQL "SELECT MIN(number_guesses) FROM users INNER JOIN games ON users.user_id = games.user_id WHERE username='$USERNAME'")
